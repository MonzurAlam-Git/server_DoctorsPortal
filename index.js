...
async function run() {
    try {
        ...
        function verifyJWT(req, res, next) {
            ...
            if (!authHeader) {
                return res.status(401).send({ message: "You dont have permission to access this" })
            }
            // token splitting
            ...
            jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
                {
                    err ? res.status(403).send({ message: 'Invalid Token' }) : req.decoded = decoded;
                    next();
                }
            })
        }
        ...
        app.put('/user/:email', async (req, res) => {
            ...
            const update = await userCollection.updateOne(filter, userUpdated, options)
        ...
        app.get('/available', async (req, res) => {
            ...
            allServices.forEach(service => {
                const filteredServices = queryServices.filter(queryService => queryService.serviceName === service.name);
